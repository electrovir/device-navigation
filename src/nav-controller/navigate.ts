...
import {NavNode, NavNodeParent, NavRootNode, buildNavTree} from '../nav-tree/nav-tree';
...
export function navigate(
    /** The HTML element from which to build the nav tree and navigate around inside of. */
    ...
    const allowsFromWrapping = allowWrapping ? true : !requiresWrapping;

    if (nextNode?.element === currentlyFocused.node.element) {
        return {
            success: false,
            reason: 'no other nodes to select',
        };
    } else if (nextNode && allowsFromWrapping) {
...
function calculateNextNode(
    ...
    if (isVertical) {
        ...
        if (!currentRow) {
            throw new Error(`No current row found at y index: '${currentlyFocused.coords.y}'`);
        }
